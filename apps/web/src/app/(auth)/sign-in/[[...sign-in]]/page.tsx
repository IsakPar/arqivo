    ...
    return (
      <div className="min-h-screen grid place-items-center p-6">
        <div className="max-w-md rounded-2xl border border-white/30 bg-white/60 p-6 shadow-xl backdrop-blur-xl ring-1 ring-white/30 text-center">
          ...
          <p className="mt-2 text-sm text-gray-700">Set NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY to enable sign in.</p>
          <Link href="/" className="mt-4 inline-block text-sm text-gray-900 underline">Back to home</Link>
        </div>
      </div>
    );
  }
  const SignIn: any = dynamic(() => import('@clerk/nextjs').then(m => m.SignIn as any), { ssr: false });
  return (
    <div className="min-h-screen grid place-items-center p-6">
      <SignIn routing="path" path="/sign-in" signUpUrl="/sign-up" afterSignInUrl="/workspace" />
    </div>
  );
}
